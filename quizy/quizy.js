...
const seikou2 = document.createTextNode("正解は「たかなわ」です"); /*そのうちletに変える。変数でも理解できる。つまりidはひとつなので*/
...
for (let i = 0; i < 10; i++) {
  const h =
    ...
    + '<div class="btn">'
    + '<ul>'
    + `<li  id=${"miss[i]"}>${sentakusi[i][0]}</li>`
    + `<li  id="success">${sentakusi[i][1]}</li>`
    + `<li  id="miss2">${sentakusi[i][2]}</li>`
